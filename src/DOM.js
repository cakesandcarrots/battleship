...
export function gamedisplay() {
  ...
  turndisplay.textContent = "Setup Phase"; 
  ...
  jackClusterBtn.addEventListener("click", () =>
    handleActivateClusterMode("Jack")
  );
  jackSalvoBtn.addEventListener("click", () =>
    handleActivateRandomSalvo("Jack")
  );
  johnClusterBtn.addEventListener("click", () =>
    handleActivateClusterMode("John")
  );
  johnSalvoBtn.addEventListener("click", () =>
    handleActivateRandomSalvo("John")
  );
}
 ...
 */
function showPlacementScreen(playerName, playerBoardData) {
...
function createDraggableShipElement(shipData, index) {
  ...
  shipVisualEl.addEventListener("touchstart", handleTouchStart, {
    passive: false,
  }); // Use passive: false if we preventDefault
...
function createBoardGrid(playerboard, boardClass, isPlacementMode = false) {
  ...
  if (!playerGridContainer) {
    console.error(
      `Grid container not found for selector: ${targetContainerSelector}`
    );
    return;
  }
  ...
  playerboard.forEach((row, i) => {
    ...
    row.forEach((cellData, j) => {
      ...
      if (isPlacementMode) {
        ...
        colcell.addEventListener("touchmove", handleTouchMove, {
          passive: false,
        }); // Need to prevent scroll
        ...
        if (cellData === "T") {
          ...
          if (gameState === "attacking") {
            ...
            const isOpponentBoard =
              (currentPlayerName === "Jack" && boardClass === "johnclass") ||
...
function handleDrop(e) {
  ...
  if (placementResult === true) {
    console.log(
      `Placed ${draggedShip.length}-length ship (${draggedShip.orientation}) at (${x},${y})`
    );
    ...
    for (let i = 0; i < draggedShip.length; i++) {
      ...
      const placedCell = document.querySelector(
        `.placement-grid-container .${boardClass} [data-xcords="${currentX}"][data-ycords="${currentY}"]`
      );
      if (placedCell) {
...
function handleTouchStart(e) {
  ...
  shipClone.style.zIndex = "1000"; // to Ensure it's on top
  shipClone.style.opacity = "0.7";
  shipClone.style.pointerEvents = "none"; //clone shouldn't interfere with events
  ...
  shipClone.style.left = `${
    e.touches[0].clientX - (touchStartX - rect.left)
  }px`;
...
function handleTouchMove(e) {
  ...
  if (
    elementUnderTouch &&
    elementUnderTouch.classList.contains("colcell") &&
    elementUnderTouch.closest(".placement-grid-container")
  ) {
...
function handleTouchEnd(e) {
  ...
  if (
    ...
    if (placementResult === true) {
      console.log(
        `Touch Placed ${touchedShip.length}-length ship (${touchedShip.orientation}) at (${x},${y})`
      );
      ...
      for (let i = 0; i < touchedShip.length; i++) {
        let currentX = x,
          currentY = y;
        if (touchedShip.orientation === "vertical") currentX += i;
        else currentY += i;
        const placedCell = document.querySelector(
          `.placement-grid-container .${boardClass} [data-xcords="${currentX}"][data-ycords="${currentY}"]`
        );
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (x >= 10 || y >= 10) {
      ...
      const cellEl = document.querySelector(
        `.placement-grid-container [data-xcords="${x}"][data-ycords="${y}"]`
      );
...
function handleConfirmPlacement() {
  ...
  const placedCells = document.querySelectorAll(
    ".placement-grid-container .colcell.ship-placed"
  );
  ...
  if (gameState === "placement_jack") {
    ...
    console.warn(
      `handleConfirmPlacement called with unexpected gameState: ${gameState}`
    );
  }
}
//chked
function startGame() {
  ...
  console.log(
    "Jack's board before startGame grid creation:",
    JSON.stringify(jack.board)
  );
  console.log(
    "John's board before startGame grid creation:",
    JSON.stringify(john.board)
  );
...
function handleActivateClusterMode(playerName) {
  ...
  const player = playerName === "Jack" ? jack : john;
  const opponent = playerName === "Jack" ? john : jack; // Get opponent object
...
function handleActivateRandomSalvo(playerName) {
  ...
  const results = player.activateRandomSalvo(opponent);
...
function processAttackResults(results, targetBoardClass) {
  ...
  results.forEach(({ x, y, result }) => {
    const cell = document.querySelector(
      `.${targetBoardClass} [data-xcords="${x}"][data-ycords="${y}"]`
    );
    if (!cell) return;
...
function handleCellClick(e) {
  ...
  const isOwnBoard =
    (currentPlayerName === "Jack" && targetBoardClass === "jackclass") ||
  ...
  if (isClusterModeActive) {
    ...
    console.log(
      `${currentPlayerName} firing Cluster Missile at (${xcords}, ${ycords})`
    );
    ...
    const results = player.activateClusterMissile(opponent, xcords, ycords);
...
function updateControlStates() {
  ...
  jackClusterBtn.disabled =
    !isJackTurn || !jack.hasCluster || isClusterModeActive;
  jackSalvoBtn.disabled = !isJackTurn || !jack.hasSalvo || isClusterModeActive;
  johnClusterBtn.disabled =
    isJackTurn || !john.hasCluster || isClusterModeActive;
